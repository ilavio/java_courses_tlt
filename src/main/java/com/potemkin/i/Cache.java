 ...
 * линейное. Массив используется для хранения элементов списка, он всегда не
 * меньше размера списка. При заполнении емкости списка, при дальнейшей вставке
 * происходит удаление первого элемента списка и вставка в последнюю ячейку.
 * Емкость списка не может быть увеличена в размере. Данный класс не
 * синхронизирован.
 * 
 * @author Илья Пот
...
public class Cache<T> {
    ...
    public Cache(int capacity) {
        if (capacity < 1) {
            log.debug("Размер массива ниже нуля:{}", capacity);
    ...
    public void add(T element, int index) {
        if (nextItem > (cache.length - 1)) {
            ...
            cache[nextItem] = new CacheElement<T>(index, element);
            nextItem += 1;
        }
    }
